...
const StaffPaymentPage: React.FC = () => {
  ...
  const {
    workProgressList,
    selectedWorkProgress: detailedWorkProgress,
    listLoading,
    detailLoading,
    error,
  ...
  // Modal states
  const [processPaymentModalVisible, setProcessPaymentModalVisible] =
    useState(false);
  ...
  const handleStatusFilterChange = (value: string) => {
    ...
    if (value === "all") {
      ...
      newFilters.currentStatus =
        value as WorkProgressQueryParams["currentStatus"];
    }
  ...
  const getStatistics = () => {
    ...
    const completed = workProgressList.filter(
      (wp) => wp.currentStatus === "completed"
    ).length;
    const inProgress = workProgressList.filter(
      (wp) => wp.currentStatus === "in_progress"
    ).length;
    const pending = workProgressList.filter(
      (wp) =>
        wp.currentStatus === "not_started" ||
        wp.currentStatus === "quote_provided"
    ).length;

    // Include amount from appointment payment if not present in paymentDetails
    const totalRevenue = workProgressList.reduce((sum, wp) => {
      const appt = wp.appointmentId;
      // Narrow appointment object without using any
      const appointmentObj =
        appt && typeof appt === "object"
          ? (appt as { payment?: { status?: string; amount?: number } })
          : null;
      const appointmentPaidAmount =
        appointmentObj?.payment?.status === "paid"
          ? appointmentObj?.payment?.amount || 0
          : 0;
      const paidAmount =
        wp.paymentDetails?.paidAmount ?? appointmentPaidAmount ?? 0;
...
const WorkProgressCard: React.FC<WorkProgressCardProps> = ({
  ...
  // Precompute paid state from either work progress or appointment
  const appointmentPaid =
    isAppointmentObject(appointment) && appointment.payment?.status === "paid";
  const wpPaid = workProgress.paymentDetails?.paymentStatus === "paid";
  const isPaid = !!(wpPaid || appointmentPaid);
  const paidAmountToShow = wpPaid
  ...
  const shouldShowPaymentButton = () => {
    ...
    // Hide if already paid (from either source)
    if (isPaid) {
      return false;
    }

    // Otherwise allow processing payment
    return true;
  };
      ...
      <Space direction="vertical" size="small" className="w-full">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <Text className="text-sm">
              {typeof technician === "string" ? technician : technician.email}
            </Text>
          </div>
          <div>
            <Text type="secondary" className="text-xs">
            ...
            <br />
            <Text className="text-sm">
              {formatDate(workProgress.serviceDate)}
            </Text>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            {isPaid && typeof paidAmountToShow === "number" && (
              <Text className="text-sm">
                {formatCurrency(paidAmountToShow)}
              </Text>
            )}
          </div>
        </div>
        ...
        {workProgress.progressPercentage !== undefined && (
          <div>
            ...
            <div className="flex items-center gap-2">
              <Text className="text-sm">
                {workProgress.progressPercentage}%
              </Text>
              <div className="flex-1 bg-gray-200 rounded-full h-2">
                <div
                  className="bg-blue-500 h-2 rounded-full"
                  style={{
                    width: `${workProgress.progressPercentage}%`,
                  }}></div>
              </div>
            </div>
          </div>
